...
const SubscriptionManagement = () => {
  // State to manage which module is active
  const [activeModule, setActiveModule] = useState('all');

  // State for subscription packages in All Module
  ...
  // State for subscription packages in Rental Module
  ...
  const handleToggleStatus = (id, moduleType) => {
    if (moduleType === 'all') {
      setAllPackages(allPackages.map(pkg =>
        pkg.id === id ? { ...pkg, status: !pkg.status } : pkg
      ));
    } else {
      setRentalPackages(rentalPackages.map(pkg =>
        pkg.id === id ? { ...pkg, status: !pkg.status } : pkg
      ));
    }
  };
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="mb-6">
        <div className="inline-flex rounded-md overflow-hidden">
          <button
            className={`px-4 py-2 text-sm ${activeModule === 'all' ? 'bg-teal-600 text-white' : 'bg-gray-100 text-gray-700'}`}
            onClick={() => setActiveModule('all')}
          >
            All Module
          </button>
          <button
            className={`px-4 py-2 text-sm ${activeModule === 'rental' ? 'bg-teal-600 text-white' : 'bg-gray-100 text-gray-700'}`}
            onClick={() => setActiveModule('rental')}
          >
            Rental Module
          </button>
        </div>
      ...
      {/* Render the appropriate section based on activeModule */}
      {activeModule === 'all' ? (
        <AllModuleSection packages={allPackages} handleToggleStatus={(id) => handleToggleStatus(id, 'all')} />
      ) : (
        <RentalModuleSection packages={rentalPackages} handleToggleStatus={(id) => handleToggleStatus(id, 'rental')} />
      )}
    </div>
  );
};

// Component for All Module Section
const AllModuleSection = ({ packages, handleToggleStatus }) => (
  <>
    {/* Overview section */}
    <div className="mb-6">
...
// Component for Rental Module Section
const RentalModuleSection = ({ packages, handleToggleStatus }) => (
  <>
    {/* Overview section */}
    <div className="mb-6">
      <div className="flex justify-between items-center mb-2">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-blue-50 p-4 rounded-md">
          <div className="flex justify-center flex-col items-center mb-2">
            <div className="bg-blue-100 p-1 rounded mb-1">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-blue-500" viewBox="0 0 20 20" fill="currentColor">
                <path d="M5 3a2 2 0 00-2 2v2a2 2 0 002 2h2a2 2 0 002-2V5a2 2 0 00-2-2H5zM5 11a2 2 0 00-2 2v2a2 2 0 002 2h2a2 2 0 002-2v-2a2 2 0 00-2-2H5zM11 5a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V5zM11 13a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" />
              </svg>
            </div>
          ...
          <div className="text-2xl font-bold text-blue-600 text-center">$ 1,000.00</div>
        </div>

        <div className="bg-yellow-50 p-4 rounded-md">
          <div className="flex justify-center flex-col items-center mb-2">
            <div className="bg-yellow-100 p-1 rounded mb-1">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-yellow-500" viewBox="0 0 20 20" fill="currentColor">
                <path d="M5 3a2 2 0 00-2 2v2a2 2 0 002 2h2a2 2 0 002-2V5a2 2 0 00-2-2H5zM5 11a2 2 0 00-2 2v2a2 2 0 002 2h2a2 2 0 002-2v-2a2 2 0 00-2-2H5zM11 5a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V5zM11 13a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" />
              </svg>
            </div>
          ...
          <div className="text-2xl font-bold text-yellow-600 text-center">$ 500.00</div>
        </div>
      </div>
    </div>
