...
const orders = []; // No data for empty state
const tabs = ["All", "Pending", "Verified", "Denied"];
    ...
    <div className="max-w-7xl mx-auto mt-8">
      <div className="bg-white rounded-xl shadow-md p-6">
        ...
        <div className="flex items-center gap-2 mb-4">
          <span className="text-xl" role="img" aria-label="offline payments">🏪</span>
          <span className="font-semibold text-base">Verify Offline Payments</span>
          <span className="ml-2 bg-gray-100 text-xs px-2 py-1 rounded">0</span>
        </div>
        {/* Warning Banner */}
        <div className="mb-3">
          <span className="block bg-pink-50 border border-pink-200 text-pink-600 rounded px-4 py-2 text-xs">
            For Offline Payments, Please Verify If The Payments Are Safely Received To Your Account. Customer Id Not Liable If You Confirm And Deliver The Orders Without Checking Payments Transactions
          </span>
        </div>
        {/* Tabs */}
        <div className="flex items-center border-b border-gray-100 mb-4">
          {tabs.map((tab, idx) => (
            <button
              key={tab}
              className={`px-4 pb-2 pt-2 text-sm font-medium focus:outline-none ${activeTab === idx ? 'text-teal-600 border-b-2 border-teal-600' : 'text-gray-400'}`}
          ...
          <table className="min-w-full bg-transparent rounded-xl text-xs">
            <thead>
              <tr className="bg-[#f7fafd] text-gray-700">
                ...
                <th className="px-3 py-2 text-left font-medium">Verify Payment</th>
                <th className="px-3 py-2 text-center font-medium">Actions</th>
              </tr>
            </thead>
            <tbody>
              {orders.map((order, idx) => (
                <tr key={order.id} className={idx % 2 === 1 ? "bg-[#f7fafd] border-b last:border-b-0" : "bg-white border-b last:border-b-0"}>
                  ...
                  <td className="px-3 py-2 text-left whitespace-nowrap">
                    <span className="block">{order.date.split(' ')[0]}</span>
                    <span className="block text-xs text-gray-400">{order.date.split(' ')[1]}</span>
                  </td>
                  <td className="px-3 py-2 text-left whitespace-nowrap">
                    <span className="block text-[#1b2c48] font-medium">{order.customer}</span>
                    <span className="block text-teal-600 text-xs">{order.contact}</span>
                  </td>
                  <td className="px-3 py-2 text-left whitespace-nowrap">{order.paymentMethod}</td>
                  <td className="px-3 py-2 text-left whitespace-nowrap">
                    <span className="block text-[#1b2c48]">{order.paymentBy}</span>
                    <span className="block text-teal-600 text-xs">{order.paymentMethod}</span>
                  </td>
                  <td className="px-3 py-2 text-left whitespace-nowrap">
                  ...
                  <td className="px-3 py-2 text-left whitespace-nowrap">
                    {order.status === 'Canceled' && (
                      <span className="px-3 py-1 rounded-full text-xs font-semibold bg-pink-50 text-pink-600 border border-pink-200">Canceled</span>
                    )}
                  </td>
